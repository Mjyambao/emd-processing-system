...
export default function TopNav({ onLogout }) {
  ...
  const [now, setNow] = useState('')                // string, rendered only after mount
  const [session, setSession] = useState(null)      // null until client

  useEffect(() => {
    setMounted(true)

    // Read session on client only
    try {
    ...
    <header className="sticky top-0 z-20 border-b border-black/10 bg-white/80 backdrop-blur">
      <div className="mx-auto max-w-6xl px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold tracking-wide">EMD Processing System</h1>
        </div>

        <div className="flex items-center gap-4 text-sm">
          {/* Render these only after mount to avoid SSR/CSR mismatch */}
          {mounted && (
            <>
              ...
              <div className="text-black/90">
                <i className="fa-regular fa-user text-brand-red"></i> {session?.name || 'Agent'}
                <span className="text-black/60"> • </span>
                <i className="fa-solid fa-id-card-clip text-black/70"></i> {session?.agentId || 'AGT-XXXX'}
              </div>
            </>
          )}
