...
export default function Dashboard(){
  ...
  const [myRows, setMyRows] = useState([
    { pnr: 'MY987B', passenger: 'James Doe',    status: 'human',     action: 'Review SSR vs seat',          stage: 'EMD Mask Checking',      error: '' },
    { pnr: 'MY777C', passenger: 'Clara Smith',   status: 'error',     action: 'Retry deal match',            stage: 'Deal Matching',     error: 'No applicable deal table found' },
  ])
  ...
  const pushToast = (t) =>
    setToasts(prev => [...prev, { id: crypto.randomUUID?.() || String(Date.now()+Math.random()), type: 'info', ...t }])
  ...
  async function killFromAll(originalIndex) {
    ...
    if (!confirm(`Kill process for ${victim?.pnr || 'this PNR'}? This cannot be undone.`)) return
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl p-4">
        ...
        <div className="mb-3 border-b border-black/10">
          <div className="flex gap-2">
            ...
            >
              ...
              All Queue <span className="ml-1 text-black/50">({allCounts.total})</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              My Queues <span className="ml-1 text-black/50">({myCounts.total})</span>
            </button>
          </div>
        </div>
        ...
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <Chip label={`All (${counters.total})`}        active={statusFilter==='all'}        onClick={() => setStatus('all')} />
          {activeTab === TABS.ALL ? (
            <>
              <Chip label={`Processed (${counters.processed})`}  color="green"  active={statusFilter==='processed'}  onClick={() => setStatus('processed')} />
              <Chip label={`Processing (${counters.processing})`} color="yellow" active={statusFilter==='processing'} onClick={() => setStatus('processing')} />
            </>
          ) : (
            ''
          )}
          
          <Chip label={`Error (${counters.error})`}        color="red"    active={statusFilter==='error'}      onClick={() => setStatus('error')} />
          <Chip label={`Human (${counters.human})`}        color="gray"   active={statusFilter==='human'}      onClick={() => setStatus('human')} />
        </div>
        ...
        <PNRDetails
          selected={selected}
          onApprove={({pnr}) => {
            setRows(list => list.map(r => r.pnr === pnr ? { ...r, status: 'processed', action: 'NA' } : r))
